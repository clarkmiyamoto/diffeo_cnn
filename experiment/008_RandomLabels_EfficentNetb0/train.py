...
from imagenet1k_data import IMAGENET1KRandomLabels

import cmd_args
import model_mlp, model_wideresnet, model_efficentnet
...
def get_data_loaders(args, shuffle_train=True):
  if args.data == 'cifar10':
    ...
    root = '/vast/xj2173/diffeo/imagenet'
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.batch_size, shuffle=False, **kwargs)
    val_loader = torch.utils.data.DataLoader(
        IMAGENET1KRandomLabels(root=root, train=False, transform=transform,
                               corrupt_prob=args.label_corrupt_prob),
        batch_size=args.batch_size, shuffle=False, **kwargs)
...
def get_model(args):
  ...
  elif args.arch =='efficentnet-b0':
    model = model_efficentnet.init_model()
  else:
    raise Exception('Unsupported architecture: {0}'.format(args.arch))
...
def train_model(args, model, train_loader, val_loader,
  ...
  num_division = 5
  intermediate_save_points = [int(epochs / num_division * i) for i in range(1,num_division + 1)]
  ...
  for epoch in range(start_epoch, epochs):
    ...
    ### Clark's Code: Saves models weights
